...
typedef struct THLOC
{
	DATE	time;	// time stamp, GMT
	float fHigh, fLow;	
	float fOpen, fClose;	
} THLOC; // 4-stream tick, internal format
...
typedef struct ASSET
{
	...
	T2		**pTicks;	// tick pointer array, used for tick simulation in trade functions
	T2		*tick_start,*tick_end;	// tick list
	...
	var	*pVolume;	// list of volume data or quote frequencies - not used yet
	int	flags;
